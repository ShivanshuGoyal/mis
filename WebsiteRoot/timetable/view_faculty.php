<?php
	...
	require_once("../Includes/AuthFac.php"); 
...
$query_rs_faculty_time_table = sprintf("SELECT * FROM time_table_info WHERE emp_id = %s", GetSQLValueString($colname_rs_faculty_time_table, "text"));
...
$query_timeslot = "SELECT emp_id from time_table_info WHERE emp_id ='".$_SESSION['SESS_USERNAME']."'";
...
<div id="view_table">
<center>
	<h3><a href="faculty_subject.php">GO BACK</a></h3>
	<h3>Time Table For <?php echo $_SESSION['SESS_USERNAME'];?></h3>
<?php
	...
	for ($i=0; $i<5; $i++)
  	{
		...
		for($j=1; $j<=10; $j++)
		{
			...
			{    
				...
				echo "<br>";					
					mysql_select_db($database_conn_time_table, $conn_time_table);
					$query_rs_course_branch = "SELECT * FROM time_table_info WHERE emp_id ='" . $_SESSION['SESS_USERNAME'] ."' AND slot_id='" . ($i*10+$j) . "'"  ;
					...
					while ($row_course_branch= mysql_fetch_assoc($rs_course_branch)) {
						echo $row_course_branch['course_id'] . "-" . $row_course_branch['branch_id'];
						echo "<br>";
					}
