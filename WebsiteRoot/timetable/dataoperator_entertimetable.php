...
else if(isset($_SESSION['SESS_USERNAME']) && !isset($_POST['select_sections'])){
	header("Location:index.php");
}

if(isset($_POST['select_sections'])){
	$_SESSION['select_sections'] = $_POST['select_sections'];
}
...
$query_classroom="select classroom_id from sec_classroomid where sec='".$_POST['select_sections']."'";
$result_classroom=mysql_query($query_classroom) or die(mysql_error());
$classroomid=mysql_fetch_assoc($result_classroom);
...
<table width="783" border="1">
  <tr class="heading">
    ...
    <th width="129">Sec</th>
    <th width="133">Sem Type</th>
    <th width="125">Semester</th>
  </tr>
  <tr class="entry">
    <td><?php echo $_POST['facultyi'] ?></td>
    <td><?php echo $_POST['subject'] ?></td>
    <td><?php echo $_POST['select_sections'] ?></td>
    <td><?php echo $_POST['semtype'] ?></td>
...
<br />
<center>
<form id="time_table" name="time_table" method="post" action="timetablefirstyr_insert.php">
<input type='hidden' name='faculty' value="<?php echo $_POST['facultyi'] ?>">
<input type='hidden' name='subject' value="<?php echo $_POST['subject'] ?>">
<input type='hidden' name='sec' value="<?php echo $_POST['select_sections'] ?>">
<input type='hidden' name='semtype' value="<?php echo $_POST['semtype'] ?>">
<input type='hidden' name='sem' value="<?php echo $_POST['sem'] ?>">
<input type='hidden' name='classroomid' value="<?php echo $classroomid['classroom_id'] ?>">
